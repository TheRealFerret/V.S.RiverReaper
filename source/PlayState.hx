...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'the-odyssey':
				dialogue = CoolUtil.coolTextFile(Paths.txt('the-odyssey/dialogue1'));
		}
			...
			case 'river':
				{
						defaultCamZoom = 0.7;
						curStage = 'river';
						var river:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('riverstage'));
						river.antialiasing = true;
						river.scrollFactor.set(1.0, 1.0);
						river.active = false;
						add(river);
				}
			default:
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				for (susNote in 0...Math.floor(susLength))
				{
					...
					if(gottaHitNote)
						sustainNote = new Note(daStrumTime + (Conductor.stepCrochet * susNote) + Conductor.stepCrochet, daNoteData, oldNote, true, false, true);
					else
